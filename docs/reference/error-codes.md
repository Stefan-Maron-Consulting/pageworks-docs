...
| LF-FONT-GLYPHLIMIT | Error | a template + dataset combination pushes one `(family, style-variant)` past the 256-distinct-glyph ceiling for a single rendered document (single-byte simple-font encoding) | the family/variant + distinct-glyph count found vs. the ceiling |
